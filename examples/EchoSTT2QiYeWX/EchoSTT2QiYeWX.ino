...
#define WX_REQUEST_TOKEN_API \
    "https://qyapi.weixin.qq.com/cgi-bin/gettoken?corpid=%s&corpsecret=%s"
#define WX_SEND_MSG_API \
    "https://qyapi.weixin.qq.com/cgi-bin/message/send?access_token=%s"
#define WX_TEXT_CONTENT                                                        \
    "{\"touser\":\"%s\",\"msgtype\":\"text\",\"agentid\":%d,\"text\":{"        \
    "\"content\":\"%s\"},\"safe\":0,\"enable_id_trans\":0,\"enable_duplicate_" \
    "check\":0,\"duplicate_check_interval\":1800}"
            ...
            if (httpCode > 0) {
                ...
                if (httpCode == HTTP_CODE_OK ||
                    ...
                    if (errcode == 0 && strcmp((char *)&errmsg, "ok") == 0) {
                        access_token_buf =
                            WX_json_decode["access_token"].as<String>();
...
int sendWXmsg(String msg) {
    if (WiFi.status() == WL_CONNECTED) {
        ...
        sprintf(buf, WX_TEXT_CONTENT, WX_USERID, WX_AGANTID,
                (char *)msg.c_str());
